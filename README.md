...
- **环境检测与一键配置**：自动探测 `dsh`、`node`、`npm`、`pnpm`、`git`、`brew`，无 dsh 时用 npm 用户级前缀安装，无 Node 时引导安装，pnpm 优先 Corepack。
...
- **运行日志**：环境 / Web / 插件 / 会话四路日志，可筛选、搜索、导出。
- **设置**：外观模式、自动启动/停止、端口、profile、DSH_HOME、API Key、升级 dsh、重置。
- **全局体验**：⌘K 命令面板、深色/浅色主题、Toast 通知、单实例锁。
...
cd src-tauri && DSH_SHELL_USER_DATA=$(mktemp -d) DSH_SHELL_SMOKE=1 cargo run
```
...
主进程只做四件事：管理 `dsh web` 子进程、调用 `dsh plugin`、同步本地会话元数据、持久化应用设置。**上游 dsh 内核与官方 Web UI 不被修改。**

## 数据归属与安全

- 应用设置：系统应用数据目录 `settings.json`。
